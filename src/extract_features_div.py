...
def ex_features_from_pcap(f_pcap):
    """
    output: file.csv which containts nested list with features
    """

    path='/home/francesco/Desktop/UniPd/thesis/master_project/out_div/'
    ...
    #array used for knowing which are dummy pkts
    where_dummies = []
    ...
    for pkt in packets:
        if IP in pkt:
            try:
                ...
                '''
                if Raw in pkt:
                    if "dummy" in str(pkt[Raw]):
                        #print("[D] Dummy")
                        where_dummies.append(pkt[IP].len)
                    else:
                        #print("[D] not Dummy")
                        where_dummies.append(-100000)
                else:
                    where_dummies.append(-100000)
                '''
                ...
                if prev_pktTime == -1:
                    ...
                    inter_times.append(pktTime-prev_pktTime)
    ...
    plt.ylabel('#pkt')
    plt.title('Density packets w/ Divergent')
    plt.show()


    #plot true+dummy packets
    plt.scatter([ i for i in range(len(pktBytes))],pktBytes, color="b", label="true pkts")
    plt.scatter([ i for i in range(len(pktBytes))],where_dummies, color="r", label="dummy pkts")
    plt.ylim(bottom=-2000 )
    plt.xlabel('#pkt')
    plt.ylabel('pkt_len [Byte]')
    plt.title('Pkt length per packet')
    plt.show()
    ...
    features.append(inter_times)
    features.append(densities)
    #TODO: compute such feature
    #features.append(bursts_bandwidth)


    print("[i] out features:"+"./features_div/"+filename+ext_csv)
    with open("features_div/"+filename+ext_csv, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerows(features)



ex_features_from_pcap(sys.argv[1])
